...
class SafeGradientFrame(tk.Canvas):
    """A canvas with a vertical gradient background."""
    ...
    def __init__(self, parent, color1, color2, **kwargs):
        ...
        except Exception as e:  # pragma: no cover - tkinter fallback
...
class SafeProgressFrame(ttk.Frame):
    """Progress frame with counters and status."""
    ...
    def _create_widgets(self):
        try:
            ...
            self.ocr_label = ttk.Label(
                status_frame,
                text="(OCR Active)",
                foreground=BRAND_COLORS["warning"],
            )
            for i in range(5):
