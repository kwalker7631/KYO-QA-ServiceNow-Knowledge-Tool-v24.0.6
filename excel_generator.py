...
FAILED_FILL = PatternFill(start_color="9C0006", end_color="9C0006", fill_type="solid")        # Dark Red
...
def apply_excel_styles(worksheet, df):
    ...
    status_colors = {'Needs Review': NEEDS_REVIEW_FILL, 'OCR Required': OCR_FILL, 'Failed': FAILED_FILL, 'Success': SUCCESS_FILL}
    ...
    for index, data_row in df.iterrows():
        ...
        if fill_color.fill_type:
            for cell in worksheet[index + 2]: cell.fill = fill_color

    for cell in worksheet[1]: cell.font = header_font
...
def generate_excel(all_results, output_path, template_path):
    try:
        ...
        for col in headers:
            if col in df_sanitized.columns: final_df[col] = df_sanitized[col]

        internal_cols = ["needs_review", "processing_status"]
        df_to_save = final_df.drop(columns=[col for col in internal_cols if col in final_df.columns], errors='ignore')
